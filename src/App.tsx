...
function App() {
  ...
  const uploadAudio = async (audioData: any) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8000/gen/upload', { base64: data });

      if (response.data.message === "Audio Converted") {
        const uploadResponse = await axios.post('/admin/input/voice_upload');
       // @ts-ignore 
            ...
            <div className="flex items-center space-x-2 p-3 bg-gray-900/90 backdrop-blur-xl rounded-2xl border border-gray-700/50 shadow-2xl">
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  className={`p-3 rounded-xl transition-all duration-300 ${recording
                ...
                <button
                  type="submit"
                  disabled={isLoading || !order.trim() || recording}
                  className={`px-6 py-3 rounded-xl flex items-center space-x-2 transition-all duration-300 ${isLoading || !order.trim() || recording
